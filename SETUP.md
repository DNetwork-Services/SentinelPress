...
| `PEXELS_API_KEY` | Free key from **pexels.com/api** (instant, no card) | Real stock photo per slide, matched to that slide's mood/topic. Falls back to a generated gradient background automatically if unset or a fetch fails. |
